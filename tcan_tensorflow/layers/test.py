...
q1 = tf.keras.layers.Input(shape=(q_dim, m_dim))
v1 = tf.keras.layers.Input(shape=(v_dim, m_dim))
k1 = tf.keras.layers.Input(shape=(v_dim, m_dim))
o1, s1 = SparseAttention(alpha=alpha)(inputs=[q1, v1, k1], return_attention_scores=True)
m1 = tf.keras.models.Model([q1, v1, k1], [o1, s1])
y1, w1 = m1([q, v, k])

# Untrained attention.
tf.random.set_seed(seed)
q2 = tf.keras.layers.Input(shape=(q_dim, m_dim))
v2 = tf.keras.layers.Input(shape=(v_dim, m_dim))
k2 = tf.keras.layers.Input(shape=(v_dim, m_dim))
o2, s2 = tf.keras.layers.Attention()(inputs=[q2, v2, k2], return_attention_scores=True)
m2 = tf.keras.models.Model([q2, v2, k2], [o2, s2])
y2, w2 = m2([q, v, k])
...
k1 = tf.keras.layers.Input(shape=(v_dim, m_dim))
o1, s1 = SparseAttention(alpha=alpha)(inputs=[q1, v1, k1], return_attention_scores=True)
m1 = tf.keras.models.Model([q1, v1, k1], o1)
...
k2 = tf.keras.layers.Input(shape=(v_dim, m_dim))
o2, s2 = tf.keras.layers.Attention()(inputs=[q2, v2, k2], return_attention_scores=True)
m2 = tf.keras.models.Model([q2, v2, k2], o2)
